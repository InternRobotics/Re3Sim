...
from policy import ACTPolicy, ACTPolicyDinov2, ACTPolicyDinov2Condition
...
def make_policy(policy_config):
    # if policy_config["backbone"] == "dino_v2" or policy_config["backbone"][:7] == "dinov2_":
    #     policy = ACTPolicyDinov2(policy_config)
    # else:
    #     policy = ACTPolicy(policy_config)
    policy = ACTPolicyDinov2Condition(policy_config)
    return policy
...
def forward_pass(data, policy):
    image_data, qpos_data, action_data, is_pad, lang_emb = data
    image_data, qpos_data, action_data, is_pad, lang_emb = (
        image_data.cuda(),
        qpos_data.cuda(),
        action_data.cuda(),
        is_pad.cuda(),
    ...
    return policy(
        qpos_data, image_data, action_data, is_pad, lang_emb
    )  # TODO remove None
...
def main(cfg: TrainingConfig):
    ...
    train_dataloader, val_dataloader, stats, _, train_dataset = load_data(
        dataset_dir,
        name_filter,
        camera_names,
        batch_size_train,
        ...
        relative_control=relative_control,
        dataset_cls="EpisodicDatasetLmdbCondition",
