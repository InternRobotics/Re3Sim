...
class EpisodicDatasetLmdbExpandLastStep(torch.utils.data.Dataset):
    @staticmethod
            ...
            try:
                with lmdb.open(
                    ...
                    with lmdb_env.begin(write=False) as txn:
                        ...
                        qvel = np.array(
                            pickle.loads(txn.get("observations/qvel".encode("utf-8")))
                        )
                        # 读取action数据
                        action = np.array(
    ...
    def __init__(
        ...
        super(EpisodicDatasetLmdbExpandLastStep).__init__()
    ...
    def get_item_single_item(self, start_ts, dataset_path):
        try:
            with lmdb.open(
                str(Path(dataset_path).parent / "lmdb"), lock=False
            ) as lmdb_env:
                with lmdb_env.begin(write=False) as txn:
                    is_sim = False
                    compressed = False
                    ...
                    qvel = pickle.loads(txn.get("observations/qvel".encode("utf-8")))[
                        start_ts
                    ]
                    image_dict = dict()
        ...
        try:
            ...
            with lmdb.open(lmdb_path, lock=False) as lmdb_env:
                with lmdb_env.begin(write=False) as txn:
                    ...
                    qvel = pickle.loads(txn.get(f"observations/qvel".encode("utf-8")))[
                        start_ts
                    ]
                    ...
                    for cam_name in self.camera_names:
                        ...
                        if render_image_bytes is not None:
                            render_image = np.array(
                                pickle.loads(render_image_bytes)
                            ).reshape(np.array(sim_image).shape)
                        else:
                            # 读取普通渲染图像
            ...
            padded_action[:action_len] = action
            padded_action[action_len:] = action[-1]
            is_pad = np.zeros(self.max_episode_len)
            is_pad[action_len:] = 1
    ...
    def __getitem__(self, index):
        ...
        if not hasattr(self, "_name_to_id"):
            ...
            with open(Path(dataset_origin_path).parent / "info.json", "r") as f:
                ...
                dist.print0(f"The number of mask-free items: {self.mask_free_items}")
