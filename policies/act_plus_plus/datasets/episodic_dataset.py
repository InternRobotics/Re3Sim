...
class EpisodicDataset(torch.utils.data.Dataset):
            ...
            with h5py.File(dataset_path, "r") as root:
                ...
                for cam_name in self.camera_names:
                    if cam_name in root["observations/render_images"].keys():
                        render_image = root[f"/observations/render_images/{cam_name}"][
                            start_ts
                        ]
                    else:
                        render_image = root[
                    ...
                    sim_image = root[f"/observations/sim_images/{cam_name}"][start_ts]
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, "r") as root:
            if "images" not in root["observations"].keys():
