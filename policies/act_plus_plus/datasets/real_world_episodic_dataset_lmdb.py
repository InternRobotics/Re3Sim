...
class RealWorldEpisodicLmdbDataset(torch.utils.data.Dataset):
    @staticmethod
    def get_norm_stats(dataset_path_list):
        ...
        for dataset_path in dataset_path_list:
            original_dataset_path = os.path.realpath(dataset_path)
            try:
                with lmdb.open(
                    str(Path(original_dataset_path).parent / "lmdb"), lock=False
                ) as env:
                    with env.begin(write=False) as txn:
                        raw_qpos = pickle.loads(
                            txn.get(f"observations/qpos".encode("utf-8"))
                        )
                        qpos = raw_qpos[:-1]  # remove last dim
                        # action
                        action = raw_qpos[1:, :]
                        gripper_action = raw_qpos[1:, -2:]
                        gripper_action = np.where(gripper_action > 0.03, 0.04, 0.0)
                ...
                print(f"Error loading {original_dataset_path} in get_norm_stats")
                print(e)
                traceback.print_exc()
                error_occurred = True
                # quit()
    ...
    def __init__(
        ...
        super(RealWorldEpisodicLmdbDataset).__init__()
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f"Error loading {original_dataset_path} in __getitem__")
            raise e
