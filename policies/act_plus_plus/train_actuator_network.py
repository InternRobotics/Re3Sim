...
def main():
    ...
    ckpt_dir = (
        f"/scr/tonyzhao/train_logs/{expr_name}"
        if os.getlogin() == "tonyzhao"
        else f"./ckpts/{expr_name}"
    )
    dataset_dir = (
        "/scr/tonyzhao/compressed_datasets/aloha_mobile_fork/"
        if os.getlogin() == "tonyzhao"
        else "/home/zfu/data/aloha_mobile_fork/"
    )
    #########################################################
    ...
    wandb.init(
        project="mobile-aloha2", reinit=True, entity="hxs001", name=expr_name
    )  # mode='disabled',
        ...
        if step % validate_every == 0:
            ...
            with torch.inference_mode():
                ...
                for batch_idx, data in enumerate(val_dataloader):
                    ...
                    out, forward_dict = policy(
                        observed_speed.cuda(), commanded_speed.cuda()
                    )
...
def visualize_prediction(
    ...
    for i, dataset_path in enumerate(vis_path):
        ...
        norm_observed_speed = (
            observed_speed - norm_stats["observed_speed_mean"]
        ) / norm_stats["observed_speed_std"]
        out_unnorm_fn = (
            lambda x: (x * norm_stats["commanded_speed_std"])
            + norm_stats["commanded_speed_mean"]
        )
        ...
        for t in range(0, episode_len, prediction_len):
            ...
            policy_input = norm_observed_speed[
                offset_start_ts - history_len : offset_start_ts + future_len
            ]
            policy_input = (
                torch.from_numpy(policy_input).float().unsqueeze(dim=0).cuda()
            )
...
class ActuatorNetwork(nn.Module):
    ...
    def forward(self, src, tgt=None):
        if tgt is not None:  # training time
            ...
            out = out[
                0 : self.prediction_len
            ]  # take first few tokens only for prediction
...
def get_norm_stats(dataset_path_list):
    ...
    stats = {
        "commanded_speed_mean": commanded_speed_mean.numpy(),
        "commanded_speed_std": commanded_speed_std.numpy(),
        "observed_speed_mean": observed_speed_mean.numpy(),
        "observed_speed_std": observed_speed_std.numpy(),
    }
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            with h5py.File(dataset_path, "r") as root:
                commanded_speed = root["/base_action"][()]
                observed_speed = root["/obs_tracer"][()]
                observed_speed = np.concatenate(
                    [self.history_pad, observed_speed, self.future_pad], axis=0
                )
                commanded_speed = np.concatenate(
                    [commanded_speed, self.prediction_pad], axis=0
                )

                offset_start_ts = start_ts + self.history_len
                commanded_speed = commanded_speed[
                    start_ts : start_ts + self.prediction_len
                ]
                observed_speed = observed_speed[
                    offset_start_ts
                    - self.history_len : offset_start_ts
                    + self.future_len
                ]
            ...
            commanded_speed = (
                commanded_speed - self.norm_stats["commanded_speed_mean"]
            ) / self.norm_stats["commanded_speed_std"]
            observed_speed = (
                observed_speed - self.norm_stats["observed_speed_mean"]
            ) / self.norm_stats["observed_speed_std"]
