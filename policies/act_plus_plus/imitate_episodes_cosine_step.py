...
def train_bc(train_dataloader, val_dataloader, config, logger_writter):
    ...
    num_steps = config["num_steps"] // dist.get_world_size()
    ckpt_dir = config["ckpt_dir"]
    seed = config["seed"]
    policy_config = config["policy_config"]
    eval_every = config["eval_every"] // dist.get_world_size()
    ...
    dist.print0("Using lr scheduler: ", config["lr_scheduler"])
    ...
    if dist.get_rank() == 0:
        if USE_GT:
            iterator = gt.timed_for(iterator)
        else:
    ...
    for step in iterator:
        ...
        if step % validate_every == 0 and step != 0:
            ...
            if dist.get_rank() == 0:
                for k, v in validation_summary.items():
                    logger_writter.add_scalar(k, v, global_step=step)

            dist.print0(f"Val loss:   {epoch_val_loss:.5f}")
            summary_string = ""
            for k, v in validation_summary.items():
                if k[0] != "_":
        ...
        optimizer.zero_grad()
        lr_scheduler.step()
        mystamp("train mode and zero_grad")
        data = next(train_dataloader)
        mystamp("get_data")
        forward_dict = forward_pass(data, policy)
        mystamp("forward_pass")
        # backward
        ...
        if dist.get_rank() == 0:
            ...
            if step % save_every == 0:
                ckpt_path = os.path.join(
                    ckpt_dir, f"policy_step_{step}_seed_{seed}.ckpt"
                )
                torch.save(policy.model.module.state_dict(), ckpt_path)
                # Define the pattern for matching the specific checkpoint files
                ckpt_pattern = re.compile(r"^policy_step_(\d+)_seed_(\d+)\.ckpt$")

                # List all matching files based on the pattern and sort them by step
                existing_ckpts = [
                ...
                if len(sorted_ckpts) > 5 and not SAVE_ALL_EPISODES:
                    ...
                    for ckpt in sorted_ckpts[:-5]:
                        ...
                        elif step_num % 1000 == 0 and step_num > (num_steps * 0.9):
                            checkpoints_to_keep.append(ckpt)
                        else:  # 否则删除
                            os.remove(os.path.join(ckpt_dir, ckpt))
        mystamp("save_ckpt")
        if USE_GT:
            dist.print0(gt.report())
