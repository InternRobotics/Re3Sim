...
def mix_loaders(train_dataloader, real_train_dataloader, sim_real_ratio=2):
    for data in train_dataloader:
        yield data
    for _ in range(sim_real_ratio):
        for data in real_train_dataloader:
            yield data
...
def train_bc(
    ...
    steps_per_epoch = (
        len(train_dataloader) + len(real_train_dataloader) * sim_real_ratio
    )
    ...
    for epoch in range(num_epochs):
        ...
        mix_loader = mix_loaders(
            train_dataloader, real_train_dataloader, sim_real_ratio
        )

        for batch_idx, data in enumerate(mix_loader):
            optimizer.zero_grad()
...
def main(cfg: TrainingConfig):
    ...
    real_task_name = args["real_task_name"]
    ...
    real_task_config = SIM_TASK_CONFIGS[real_task_name]
    real_camera_names = real_task_config["camera_names"]
    real_dataset_dir = real_task_config["dataset_dir"]
    sim_real_ratio = args["sim_real_ratio"]
    ...
    real_config = {
        ...
        "actuator_config": actuator_config,
        "relative_control": relative_control,
        "dataset_cls": args["real_dataset_cls"],
        "lr_scheduler": args["lr_scheduler"],
        "num_warmup_steps": args["num_warmup_steps"],
    }
