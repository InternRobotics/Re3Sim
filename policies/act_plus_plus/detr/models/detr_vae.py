...
class DETRVAE(nn.Module):
    ...
    def forward(
        ...
        return a_hat, None, [mu, logvar], probs, binaries
    ...
    def forward(
        ...
        if self.backbones is not None:
            ...
            featuress = featuress[0].view(
                image.shape[0], len(self.camera_names), featuress[0].shape[1], 16, 22
            )  # take the last layer feature
            pos = poss[0]
            for cam_id, cam_name in enumerate(self.camera_names):
                # start = time.time()
        ...
        a_hat = self.action_head(hs)
        return a_hat, None, [mu, logvar], None, None


class DETRVAEDinoCondition(nn.Module):
    """This is the DETR module that performs object detection"""
    ...
    def __init__(
        ...
        self.input_lang_emb_proj = nn.Linear(512, hidden_dim)
    ...
    def forward(
        ...
        if self.backbones is not None:
            ...
            #     all_cam_features.append(self.input_proj(features))
            #     all_cam_pos.append(pos)
            lang_emb = self.input_lang_emb_proj(lang_emb)
            # proprioception features
            proprio_input = self.input_proj_robot_state(qpos)
            # fold camera dimension into width dimension
            src = torch.cat(all_cam_features, axis=3)
            src = src + lang_emb.unsqueeze(-1).unsqueeze(-1)
            pos = torch.cat(all_cam_pos, axis=3)
            hs = self.transformer(
                src,
                None,
