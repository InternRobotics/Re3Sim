...
def main(args):
    ...
    episode_idxs = [
        ...
        if (".hdf5" in name) and ("features" not in name)
    ]
    episode_idxs.sort()
    ...
    for episode_idx in range(num_episodes):
        ...
        if not feature_extractors:
            for cam_name in camera_names:
                ...
                loading_status = resnet.load_state_dict(
                    torch.load(ckpt_path.replace("DUMMY", cam_name))
                )
        ...
        dataset_path = os.path.join(
            dataset_dir, f"{repr_type}_features_seed{seed}_episode_{episode_idx}.hdf5"
        )
        print(dataset_path)
        # HDF5
        t0 = time.time()
