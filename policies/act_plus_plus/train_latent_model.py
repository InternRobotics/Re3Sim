...
def forward_pass(data, policy, latent_model):
    ...
    with torch.no_grad():
        output_labels = F.one_hot(
            torch.argmax(output_logits, dim=-1), num_classes=gt_labels.shape[-1]
        ).float()
...
def train_bc(train_dataloader, val_dataloader, config, ckpt_name):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.inference_mode():
            ...
            if epoch_val_loss < min_val_loss:
                ...
                best_ckpt_info = (
                    epoch,
                    min_val_loss,
                    deepcopy(latent_model.state_dict()),
                )
        ...
        if epoch % 100 == 0:
            ckpt_path = os.path.join(
                ckpt_dir, f"latent_model_epoch_{epoch}_seed_{seed}.ckpt"
            )
    ...
    best_epoch, min_val_loss, best_state_dict = best_ckpt_info
    ckpt_path = os.path.join(
        ckpt_dir, f"latent_model_epoch_{best_epoch}_seed_{seed}.ckpt"
    )
    torch.save(best_state_dict, ckpt_path)
    print(
