...
def calculate_nearest_neighbors(
    ...
    pairwise_dist_vis = torch.norm(
        curr_vis_feature - support_vis_feature, dim=1
    ).unsqueeze(0)
...
def main(args):
    ...
    for episode_id in range(0, 40):
        ...
        for cam_name in camera_names:
            feature_dataset_path = os.path.join(
                dataset_dir,
                f"{repr_type}_features_seed{seed}_episode_{episode_id}.hdf5",
            )
        ...
        eps_len = len(action)
        indices = np.tile(np.arange(skip), eps_len).reshape(
            eps_len, skip
        )  # each row is 0, 1, ... skip
    ...
    Y = np.concatenate(Y)
    train_inputs = [
        torch.from_numpy(vis_features).cuda(),
        torch.from_numpy(state_features).cuda(),
    ]
    ...
    for cam_name in camera_names:
        ...
        loading_status = resnet.load_state_dict(
            torch.load(model_dir.replace("DUMMY", cam_name))
        )
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            for t in range(sim_episode_len):
                ...
                if t % query_freq == 0:
                    ...
                    for cam_id, curr_image in enumerate(curr_image_raw):
                        ...
                        curr_image_feature = feature_extractors[camera_names[cam_id]](
                            curr_image
                        )
                    ...
                    action = calculate_nearest_neighbors(
                        curr_feature, train_inputs, train_targets, k, state_weight
                    )  # TODO use this
                    action = action.squeeze(0).cpu().numpy()
                    action = np.concatenate(
                        [action[:-BASE_DELAY, :-2], action[BASE_DELAY:, -2:]], axis=1
                    )
            ...
            plt.close()
        if real_robot:
            env.puppet_bot_left.dxl.robot_set_operating_modes(
                "single", "gripper", "position"
            )
            env.puppet_bot_right.dxl.robot_set_operating_modes(
                "single", "gripper", "position"
            )
            env.puppet_bot_left.dxl.robot_set_operating_modes(
                "single", "gripper", "pwm"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir", action="store", type=str, help="model_dir", required=True
    )
    parser.add_argument(
        "--task_name", action="store", type=str, help="task_name", required=True
    )
    parser.add_argument(
        "--ckpt_dir", action="store", type=str, help="The text to parse.", required=True
    )
