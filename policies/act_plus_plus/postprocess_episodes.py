...
def load_hdf5(dataset_dir, dataset_name):
    ...
    if compressed:
        for cam_id, cam_name in enumerate(image_dict.keys()):
            ...
            image_list = []
            for (
                padded_compressed_image
            ) in padded_compressed_image_list:  # [:1000] to save memory
...
def main(args):
    ...
    for episode_idx in range(start_idx, start_idx + num_episodes):
        ...
        qpos, qvel, action, base_action, image_dict, is_sim = load_hdf5(
            dataset_dir, dataset_name
        )

        # process proprioception
        qpos = np.concatenate(
            [qpos[:, 7:] * MIRROR_STATE_MULTIPLY, qpos[:, :7] * MIRROR_STATE_MULTIPLY],
            axis=1,
        )
        qvel = np.concatenate(
            [qvel[:, 7:] * MIRROR_STATE_MULTIPLY, qvel[:, :7] * MIRROR_STATE_MULTIPLY],
            axis=1,
        )
        action = np.concatenate(
            [
                action[:, 7:] * MIRROR_STATE_MULTIPLY,
                action[:, :7] * MIRROR_STATE_MULTIPLY,
            ],
            axis=1,
        )
        ...
        if "left_wrist" in image_dict.keys():
            image_dict["left_wrist"], image_dict["right_wrist"] = (
                image_dict["right_wrist"][:, :, ::-1],
                image_dict["left_wrist"][:, :, ::-1],
            )
        elif "cam_left_wrist" in image_dict.keys():
            image_dict["cam_left_wrist"], image_dict["cam_right_wrist"] = (
                image_dict["cam_right_wrist"][:, :, ::-1],
                image_dict["cam_left_wrist"][:, :, ::-1],
            )
        else:
        ...
        with h5py.File(dataset_path + ".hdf5", "w", rdcc_nbytes=1024**2 * 2) as root:
            ...
            if COMPRESS:
                _ = root.create_dataset(
                    "compress_len", (len(image_dict.keys()), max_timesteps)
                )
        ...
        if episode_idx == start_idx:
            save_videos(
                ...
                video_path=os.path.join(
                    dataset_dir, dataset_name + "_mirror_video.mp4"
                ),
            )
