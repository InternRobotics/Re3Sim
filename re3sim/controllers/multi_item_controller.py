...
class MultiItemController(BaseController):
    def __init__(
        self,
        name: str,
        items: List[Item],
        camera_params: Union[List[float | int], np.ndarray],
        hand_camera_prim_path: str,
        root_prim_path: str,
        gripper: ParallelGripper = None,
        gripper_joint_opened_positions: np.ndarray = None,
        gripper_joint_closed_positions: np.ndarray = None,
        ...
        bg_collision_mesh_offset: float = 0.0,
        ...
        # settings
        if gripper is None:
            self._gripper_joint_closed_positions = gripper_joint_closed_positions
            self._gripper_joint_opened_positions = gripper_joint_opened_positions
        else:
            self._gripper_joint_opened_positions = gripper.joint_opened_positions
            self._gripper_joint_closed_positions = gripper.joint_closed_positions
        ...
        self._hand_camera_prim_path = hand_camera_prim_path
        ...
        # collision mesh
        pcd_path = os.path.join(tmp_dir, "pcd_bg.npz")
    ...
    def forward(
        ...
        Returns:
            np.ndarray: action to be executed by the ArticulationController
        """
        ...
        if not self.is_done():
            ...
            if (
                ...
                if not self.milestone["at_the_cube"]:
                    ...
                    if hasattr(self, "post_pick_position"):
                        if not self.plan_results and self.target_reached(
                            gripper_position, self.post_pick_position
                        ):
                    ...
                    if not self.plan_results and self.target_reached(
                        gripper_position,
                        placing_position
                        + self.items[self._item_idx].params["place_height_offset"],
                    ):
    ...
    def get_target_pose(
        ...
        if not self.milestone["reach_post_position"]:
            ...
            self.post_pick_position = (
                np.array([pose.p[0], pose.p[1], pose.p[2]]) + self._post_position_rela
            )
            pose = mplib.Pose(p=self.post_pick_position, q=placing_orientation)
            return pose, True
