...
from ..envs.config import SimulatorConfig
...
class BaseEnv:
    ...
    def __init__(
        ...
        from ..envs.runner import SimulatorRunner  # noqa E402.

        self._runner = SimulatorRunner(config=config, to_log=to_log)
    ...
    def step(
        self, actions: List[Dict[str, Any]], render: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        if len(actions) == 0:
            self._runner.step({}, only_step=True)
            return {}
        ...
        self._runner.step(action_after_reshape, render=render)
    ...
    def reset(self, envs: List[int] = None):
        ...
        self._runner.reset()
        for i in range(20):
            self._runner._world.step(render=True)
        for task in self.runner.current_tasks.values():
